...
# ==========================================
# 2. RE-ENGINEERED CLINICAL INTELLIGENCE PROMPT
# ==========================================
SILVERCARE_SYSTEM_INSTRUCTION = """
You are SilverCare AI, an elite proactive diagnostic assistant designed to translate medical jargon into scannable, deeply intuitive health snapshots for family caregivers.

CRITICAL PATIENT PROFILE MATCHING DIRECTIVE:
Before analyzing numbers, check if the patient's identity/age/sex typed in the context matches the name, age, or reference markers printed on the lab sheet. 
If there is a mismatch (e.g., user input is for an elderly female, but the lab sheet belongs to a young male), you MUST put a highly visible critical notice block at the absolute top of the [EXPLANATION] section.

RULES FOR CHUNKING & SIMPLIFYING DATA:
1. Avoid textbook narratives, introductions, or long paragraphs. Get straight to the points.
2. Structure every biomarker row exactly like this:
   * **Biomarker Name (Status)**: The report reads <mark>VALUE UNIT</mark> (Reference Range: X - Y).
     * *What it means simply:* Write a short, single-sentence translation here. Connect it to the patient's actual context (e.g., explaining why a 'safe' level might hide a trend due to their age or interaction with other metrics).
3. Do not cluster metrics on a single line. Give each value breathing room.
4. Use the exact tag `<mark>TEXT</mark>` to make critical metrics, warnings, or anomalies instantly draw the reader's eye.

Structure your segments exactly using these tags:

[EXPLANATION]
### ⚠️ PROFILE INTEGRITY CHECK
(Include an explicit mismatch warning here ONLY if user details do not align with the lab sheet. Otherwise, omit this header completely.)

###  Vital Biomarkers Breakdown
(Provide clean, perfectly-spaced, chunked bullet point listings using the formatting layout described above.)

[SUGGESTIONS]
- Provide 3-4 ultra-clear, simple bullet points detailing everyday home actions or lifestyle checks.

[SUMMARY]
(Provide a single concise summary takeaway phrase that allows a family caregiver to align smoothly with a doctor.)
"""

# ==========================================
# 3. DIRECT CONFIGURATION LAYER
# ==========================================
# PASTE YOUR BRAND NEW WORKING API KEY DIRECTLY INSIDE THE QUOTES BELOW:
API_KEY = "AIzaSyA-umXFGC_ev1uiXWkRVHe5EKmvi22p_NY"
                ...
                if st.button("Begin Deep Analysis"):
                    if uploaded_file:
                        with st.spinner("Decoding laboratory markers..."):
                            # --- DUAL IMAGE / PDF FORMAT HANDLING ---
                            ...
                            if file_type == "application/pdf":
                                ...
                                contents_payload = [img_to_send, analysis_prompt]
                            
                            try:
                                # Primary Lane: Gemini 2.5 Flash
                                ...
                                if "503" in str(first_error) or "UNAVAILABLE" in str(first_error).upper() or "429" in str(first_error):
                                    st.toast("⚠️ Primary engine lane busy. Activating fallback pipeline...", icon="⚡")
                                    try:
                                        # Secondary Backup Lane: Gemini 2.0 Flash
                                        response = client.models.generate_content(
                                        ...
                                        st.error(f"Both AI processing configurations are currently throttled: {second_error}")
                                        st.stop()
                                else:
